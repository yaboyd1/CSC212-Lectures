...
namespace main_savitch_5 {
    ...
    std::size_t list_length(const node* head_ptr); /* O(n): Calculates how many nodes in linked list */

    void list_head_insert(node*& head_ptr, const node::value_type& entry); /* O(1): Inserts node at head */
    void list_insert(node* previous_ptr, const node::value_type& entry); /* O(1): Inserts node after another node */

    void list_head_remove(node*& head_ptr); /* O(1): Removes node at head */
    void list_remove(node* previous_ptr); /* O(1): Removes node after another node */

    /* O(n): Serial Search for a node with specified value */
    node* list_search(node* head_ptr, const node::value_type& target); 
    const node* list_search(const node* head_ptr, const node::value_type& target); 

    /* O(n): Finds a node at specified location */
    /* Essentially array[i] but for the linked list */
    node* list_locate(node* head_ptr, std::size_t position);
    const node* list_locate(const node* head_ptr, std::size_t position);

    void list_clear(node*& head_ptr); /* O(n): Clears the linked list */
    void list_copy(const node* source_ptr, node*& head_ptr, node*& tail_ptr); /* O(n): Copies elements of one linked list to another */
}
