...
namespace main_savitch_2C {
	...
	statistician operator +(const statistician& s1, const statistician& s2) {
		...
		return statistician(s1.count + s2.count, s1.total + s2.total, s1.tinyest < s2.tinyest ? s1.tinyest : s2.tinyest,s1.largest > s2.largest ? s1.largest : s2.largest);
	}

	statistician operator *(double scale, const statistician& s) {
		if (s.count == 0) return s;
		return statistician(s.count, scale * s.total, scale > 0 ? scale * s.tinyest : scale * s.largest, scale > 0 ? scale * s.largest : scale * s.tinyest);
	}
